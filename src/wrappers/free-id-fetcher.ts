...
export class FreeIdFetcher {
  public static async get(
    table: string,
    idFieldName: string,
    reservedFieldName: string = "name"
  ) {
    let r = await SpireApi.v1().get(
      `query/free-ids-reserved/${table}/${idFieldName}/${reservedFieldName}`
    );
    if (r.status === HttpStatus.OK) {
      ...
      r = await SpireApi.v1().get(
        `query/free-id-ranges/${table}/${idFieldName}`
      );
      if (r.status === HttpStatus.OK) {
